...
namespace xCVM
{
    ...
    class SelectedColliderGroupGUI
    {
        ...
        public SelectedColliderGroupGUI(SerializedObject so, int i)
        {
            ...
            _colliderGroupList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                ...
                if (isFocused)
                {
                    ...
                    if (id != xCVMSpringBoneCollider.SelectedGuid)
                    {
                        xCVMSpringBoneCollider.SelectedGuid = id;
        ...
        public void Draw3D()
        {
            var target = _so.targetObject as xCVMSpringBoneColliderGroup;
            ...
            foreach (var c in target.Colliders)
            {
                ...
                if (c.IsSelected)
                {
                    ...
                    {
                        Undo.RecordObject(c, "xCVMSpringBoneCollider");
                        c.Offset = offset;
    ...
    class SelectedSpringGUI : SelectedGUIBase
    {
        ...
        public SelectedSpringGUI(xCVMInstance target, SerializedObject so, int i) : base(so, i)
        {
...

            {
                ...
                _springColliderGroupList.drawElementCallback = (rect, index, isActive, isFocused) =>
                {
                    ...
                    var element_index = elements.IndexOf(element.objectReferenceValue as xCVMSpringBoneColliderGroup);
