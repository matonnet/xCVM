...
namespace xCVM
{
    ...
    {
        public xCVMInstance Target { get; private set; }
        ...
        public SpringBoneTreeView(TreeViewState state, xCVMInstance target, SerializedObject so) : base(state)
        {
            Target = target;
            _so = so;
        ...
        protected override void SelectionChanged(IList<int> selectedIds)
        {
            ...
            {
                if (value is xCVMSpringBoneColliderGroup colliderGroup)
                {
                    var i = Target.SpringBone.ColliderGroups.IndexOf(colliderGroup);
                    _selected = new SelectedColliderGroupGUI(_so, i);
                }
                else if (value is xCVMInstanceSpringBone.Spring spring)
                {
        ...
        xCVMInstanceSpringBone last = null;
        MonoBehaviour Hover = null;
        public MonoBehaviour Active = null;

        public bool Draw3D(xCVMInstanceSpringBone springBone)
        {
            ...
            if (Active is xCVMSpringBoneCollider activeCollider)
            {
                ...
                if (activeCollider.ColliderType == xCVMSpringBoneColliderTypes.Capsule)
                {
        ...
        (bool IsActive, bool IsHover) DrawCollider(xCVMSpringBoneCollider collider, Color color)
        {
        ...
        (bool IsActive, bool IsHover) DrawJoint(xCVMSpringBoneJoint joint, Color color, xCVMSpringBoneJoint head)
        {
            if (Event.current.type == EventType.Repaint || Event.current.type == EventType.Layout)
            {
