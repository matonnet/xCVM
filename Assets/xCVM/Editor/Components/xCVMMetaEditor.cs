...
{
    /// <summary>
    /// Editor for xCVMObjectMeta
    /// </summary>
    public class xCVMMetaEditor : SerializedPropertyEditor
    {
        SerializedProperty m_thumbnail;
        xCVMMetaProperty m_name;
        xCVMMetaProperty m_version;
        xCVMMetaProperty m_copyright;
        xCVMMetaProperty m_authors;
        xCVMMetaProperty m_references;
        xCVMMetaProperty m_contact;
        xCVMMetaProperty m_thirdPartyLicenses;
        xCVMMetaProperty m_OtherLicenseUrl;
        ...
        xCVMMetaProperty CreateValidation(string name, xCVMMetaPropertyValidator validator = null)
        {
            return new xCVMMetaProperty(m_rootProperty.FindPropertyRelative(name), validator);
        }

        public xCVMMetaEditor(SerializedObject serializedObject, SerializedProperty property) : base(serializedObject, property)
        {
            m_name = CreateValidation(nameof(xCVMObjectMeta.Name), prop =>
                        {
            ...
            m_version = CreateValidation(nameof(xCVMObjectMeta.Version));
            m_authors = CreateValidation(nameof(xCVMObjectMeta.Authors), prop =>
                        {
                            if (prop.arraySize == 0)
                            {
                                return (RequiredMessage(prop.name), MessageType.Error);
            ...
            m_copyright = CreateValidation(nameof(xCVMObjectMeta.CopyrightInformation));
            m_contact = CreateValidation(nameof(xCVMObjectMeta.ContactInformation));
            m_references = CreateValidation(nameof(xCVMObjectMeta.References));
            m_thirdPartyLicenses = CreateValidation(nameof(xCVMObjectMeta.ThirdPartyLicenses));
            m_OtherLicenseUrl = CreateValidation(nameof(xCVMObjectMeta.OtherLicenseUrl));
            m_thumbnail = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.Thumbnail));
            // AvatarPermission
            m_AvatarPermission = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.AvatarPermission));
            m_ViolentUssage = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.ViolentUsage));
            m_SexualUssage = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.SexualUsage));
            m_CommercialUssage = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.CommercialUsage));
            m_PoliticalOrReligiousUsage = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.PoliticalOrReligiousUsage));
            m_AntisocialOrHateUsage = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.AntisocialOrHateUsage));
            // Mod
            m_CreditNotation = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.CreditNotation));
            m_Redistribution = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.Redistribution));
            m_Modification = m_rootProperty.FindPropertyRelative(nameof(xCVMObjectMeta.Modification));

        }

        public static xCVMMetaEditor Create(SerializedObject serializedObject)
        {
            return new xCVMMetaEditor(serializedObject, serializedObject.FindProperty(nameof(xCVMObject.Meta)));
        }
