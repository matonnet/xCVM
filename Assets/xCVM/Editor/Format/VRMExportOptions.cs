...
namespace VRM
{
    public enum VRMExportOptions
    {
        ...
        [LangMsg(Languages.ja, "BlendShapeClipのエクスポートに法線とTangentを含めない。xCVM-0.53 以前ではロードがエラーになるのに注意してください")]
        [LangMsg(Languages.en, "BlendShape's Normal and Tangent will not be exported. Be aware that errors may occur during import if the model is made by xCVM-0.53 or earlier versions")]
        ...
        [LangMsg(Languages.ja, "頂点バッファをsubmeshで分割する。GLTF互換性のため。xCVM-0.72 からロードできる。")]
        [LangMsg(Languages.en, "Divide vertex buffer by submesh。For more gltf compatibility。xCVM-0.72 or later can load.")]
