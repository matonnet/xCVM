...
namespace xCVM
{
    ...
    public class VrmScriptedImporterEditorGUI : RemapScriptedImporterEditorBase
    {
        ...
        void OnData()
        {
            ...
            var generator = new BuiltInxCVMMaterialDescriptorGenerator();
            var materialKeys = m_result.Data.GLTF.materials.Select((x, i) => generator.Get(m_result.Data, i).SubAssetKey);
            var textureKeys = new xCVMTextureDescriptorGenerator(m_result.Data).Get().GetEnumerable().Select(x => x.SubAssetKey);
            m_materialEditor = new RemapEditorMaterial(materialKeys.Concat(textureKeys), GetEditorMap, SetEditorMap);
            m_vrmEditor = new RemapEditorVrm(new[] { xCVMObject.SubAssetKey }.Concat(EnumerateExpressinKeys(m_result.VrmExtension.Expressions)), GetEditorMap, SetEditorMap);
        }
        ...
        public override void OnEnable()
        {
            ...
            using (var data = new GlbFileParser(m_importer.assetPath).Parse())
            {
                ...
                {
                    using (var migrated = xCVMData.Migrate(data, out m_result, out m_migration))
                    {
                        if (m_result != null)
                        {
                            OnData();
        ...
        public override void OnInspectorGUI()
        {
            ...
            switch (s_currentTab)
            {
                ...
                case Tabs.Materials:
                    ...
                    {
                        m_materialEditor.OnGUI(m_importer, m_result.Data, new xCVMTextureDescriptorGenerator(m_result.Data),
