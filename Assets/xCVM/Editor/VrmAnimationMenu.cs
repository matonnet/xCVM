...
namespace xCVM
{
    internal static class VrmAnimationMenu
    {
        ...
        static Transform GetParentBone(Dictionary<HumanBodyBones, Transform> map, xCVMHumanoidBones bone)
        {
            while (true)
            {
                if (bone == xCVMHumanoidBones.Hips)
                {
                    break;
                }
                var parentBone = xCVMHumanoidBoneSpecification.GetDefine(bone).ParentBone.Value;
                var unityParentBone = xCVMHumanoidBoneSpecification.ConvertToUnityBone(parentBone);
        ...
        static byte[] BvhToVrmAnimation(string path)
        {
            ...
            exporter.Export((VrmAnimationExporter vrma) =>
            {
                ...
                {
                    var vrmBone = xCVMHumanoidBoneSpecification.ConvertFromUnityBone(kv.Key);
