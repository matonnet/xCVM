...
namespace xCVM
{
    public class xCVMExportDialog : ExportDialogBase
    {
        public const string MENU_NAME = "Export VRM 1.0...";

        public static void Open()
        {
            var window = GetWindow<xCVMExportDialog>(MENU_NAME);
            window.Show();
        }
        ...
        xCVMObject m_tmpObject;
        xCVMMetaEditor m_metaEditor;

        protected override void Initialize()
        {
            m_tmpObject = ScriptableObject.CreateInstance<xCVMObject>();
            m_tmpObject.name = "_vrm1_";
            m_tmpObject.Meta.Authors = new List<string> { "" };

            m_settings = ScriptableObject.CreateInstance<xCVMExportSettings>();
            ...
            State.ExportRootChanged += (root) =>
            {
                ...
                {
                    var controller = root.GetComponent<xCVMInstance>();
                    if (controller != null)
                    {
                        Vrm = controller.Vrm;
                    }
        ...
        protected override IEnumerable<Validator> ValidatorFactory()
        {
            ...
            // Mesh/Renderer のチェック
            m_meshes.MaterialValidator = new xCVMMaterialValidator();
            ...
            switch (_tab)
            {
                case Tabs.Meta:
                    if (m_metaEditor == null)
                    {
                        ...
                        m_metaEditor = xCVMMetaEditor.Create(so);
                    }
        ...
        protected override void ExportPath(string path)
        {
            ...
            try
            {
                using (var arrayManager = new NativeArrayManager())
                {
                    ...
                    // export vrm-1.0
                    var exporter = new xCVM.xCVMExporter(new EditorTextureSerializer(), m_settings.MeshExportSettings);
