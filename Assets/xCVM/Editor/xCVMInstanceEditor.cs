...
namespace xCVM
{
    [CustomEditor(typeof(xCVMInstance))]
    public class xCVMInstanceEditor : Editor
    {
        const string SaveTitle = "New folder for vrm-1.0 assets...";
        static string[] SaveExtensions = new string[] { "asset" };

        static xCVMObject CreateAsset(string path, Dictionary<ExpressionPreset, xCVMExpression> expressions, xCVMInstance instance)
        {
            ...
            var asset = ScriptableObject.CreateInstance<xCVMObject>();
            ...
            var loaded = unityPath.LoadAsset<xCVMObject>();
        ...
        static xCVMExpression CreateAndSaveExpression(ExpressionPreset preset, string dir, xCVMInstance instance)
        {
            var prefab = PrefabUtility.GetCorrespondingObjectFromOriginalSource(instance.gameObject);
            var clip = ScriptableObject.CreateInstance<xCVMExpression>();
            clip.name = preset.ToString();
            clip.Prefab = prefab;
            var path = System.IO.Path.Combine(dir, $"{preset}.asset");
            var unityPath = UnityPath.FromFullpath(path);
            unityPath.CreateAsset(clip);
            var loaded = unityPath.LoadAsset<xCVMExpression>();
            return loaded;
        }

        static string GetSaveName(xCVMInstance instance)
        {
            if (instance == null)
            {
        ...
        void SetupxCVMObject(xCVMInstance instance)
        {
            ...
            // xCVMObject
            var prop = serializedObject.FindProperty(nameof(xCVMInstance.Vrm));
            if (prop.objectReferenceValue == null)
            {
                EditorGUILayout.HelpBox("No xCVMObject.", MessageType.Error);
            }
            if (GUILayout.Button("Create new xCVMObject and default Expressions. select target folder"))
            {
                var saveName = GetSaveName(instance);
                var dir = SaveFileDialog.GetDir(SaveTitle, System.IO.Path.GetDirectoryName(saveName));
                if (!string.IsNullOrEmpty(dir))
                {
                    var expressions = new Dictionary<ExpressionPreset, xCVMExpression>();
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (target is xCVMInstance instance)
            {
                if (instance.Vrm == null)
                {
                    SetupxCVMObject(instance);
                }

                if (instance.Vrm != null)
                {
                    EditorGUILayout.HelpBox("SpringBone gizmo etc...", MessageType.Info);
                    if (GUILayout.Button("Open " + xCVMWindow.MENU_NAME))
                    {
                        xCVMWindow.Open();
                    }
                }
        ...
        {
            // 親指のガイド          
            DrawThumbGuide(target as xCVMInstance);
        }

        static void DrawThumbGuide(xCVMInstance instance)
        {
            if (instance == null)
            {
                return;
