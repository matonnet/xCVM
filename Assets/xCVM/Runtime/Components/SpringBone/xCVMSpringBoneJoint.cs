...
namespace xCVM
{
    ...
    public class xCVMSpringBoneJoint : MonoBehaviour
    {
        [SerializeField]
        ...
        void AddJointRecursive(Transform t, xCVMSpringBoneJoint src)
        {
            var joint = t.gameObject.GetComponent<xCVMSpringBoneJoint>();
            if (joint == null)
            {
                joint = t.gameObject.AddComponent<xCVMSpringBoneJoint>();
        ...
        void GetJoints(Transform t, List<xCVMSpringBoneJoint> joints)
        {
            var joint = t.GetComponent<xCVMSpringBoneJoint>();
            if (joint != null)
            {
                joints.Add(joint);
            }
        ...
        {
            var root = GetComponentInParent<xCVMInstance>();
            if (root == null)
            {
                Debug.LogWarning("not xCVMInstance");
                return;
            }
            ...
            foreach (var spring in root.SpringBone.Springs)
            {
                for (int i = 0; i < spring.Joints.Count; ++i)
                {
                    if (spring.Joints[i] == this)
                    {
                        ...
                        var joints = new List<xCVMSpringBoneJoint>();
        ...
        (xCVMInstanceSpringBone.Spring, int) FindTail(xCVMInstance vrm, xCVMSpringBoneJoint head)
        {
        ...
        {
            var vrm = GetComponentInParent<xCVMInstance>();
            if (vrm == null)
            {
                return;
            }
