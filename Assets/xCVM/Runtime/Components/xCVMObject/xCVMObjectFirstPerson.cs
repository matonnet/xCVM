...
{
    [Serializable]
    public class xCVMObjectFirstPerson
    {
        [SerializeField]
        ...
        public async Task SetupAsync(GameObject go, IAwaitCaller awaitCaller, bool isSelf = true, int? firstPersonOnlyLayer = default, int? thirdPersonOnlyLayer = default)
        {
            ...
            var layer = (
                xCVMFirstPersonLayerSettings.GetFirstPersonOnlyLayer(firstPersonOnlyLayer),
                xCVMFirstPersonLayerSettings.GetThirdPersonOnlyLayer(thirdPersonOnlyLayer));
            ...
            foreach (var x in Renderers)
            {
                ...
                {
                    // 同じ対象が複数回現れた
                    Debug.LogWarning($"xCVMObjectFirstPerson.SetupAsync: duplicated {x.Renderer}");
                    continue;
                }
