...
namespace xCVM
{
    ...
    /// </summary>
    public sealed class xCVMControlBone
    {
        /// <summary>
        ...
        private readonly List<xCVMControlBone> _children = new List<xCVMControlBone>();
        public IReadOnlyList<xCVMControlBone> Children => _children;

        private xCVMControlBone(Transform controlTarget, HumanBodyBones boneType, xCVMControlBone parent, Transform root)
        {
            ...
            ControlBone = new GameObject($"{nameof(xCVMControlBone)}:{boneType.ToString()}").transform;
        ...
        public static xCVMControlBone Build(UniHumanoid.Humanoid humanoid, out Dictionary<HumanBodyBones, xCVMControlBone> boneMap, Transform root)
        {
            var hips = new xCVMControlBone(humanoid.Hips, HumanBodyBones.Hips, null, root);
            boneMap = new Dictionary<HumanBodyBones, xCVMControlBone>();
        ...
        private static void BuildRecursively(UniHumanoid.Humanoid humanoid, Transform current, xCVMControlBone parent, Dictionary<HumanBodyBones, xCVMControlBone> boneMap, Transform root)
        {
            if (humanoid.TryGetBoneForTransform(current, out var bone))
            {
                var newBone = new xCVMControlBone(current, bone, parent, root);
                parent = newBone;
