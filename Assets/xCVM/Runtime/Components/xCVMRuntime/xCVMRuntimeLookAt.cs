...
namespace xCVM
{
    public sealed class xCVMRuntimeLookAt : ILookAtEyeDirectionProvider
    {
        private readonly xCVMObjectLookAt _lookAt;
        ...
        internal xCVMRuntimeLookAt(xCVMObjectLookAt lookAt, UniHumanoid.Humanoid humanoid, xCVMRuntimeControlRig controlRig)
        {
            _lookAt = lookAt;
        ...
        /// <summary>
        /// Yaw/Pitch 値を直接設定します。
        /// xCVMInstance.LookAtTargetTypes が SpecifiedTransform の場合、ここで設定しても値は上書きされます。
        /// </summary>
        ...
        public (float, float) GetLookAtYawPitch(
            Transform head,
            xCVMObjectLookAt.LookAtTargetTypes lookAtTargetType,
            Transform gaze)
        {
            switch (lookAtTargetType)
            {
                case xCVMObjectLookAt.LookAtTargetTypes.SpecifiedTransform:
                    return CalculateYawPitchFromLookAtPosition(gaze.position);
                case xCVMObjectLookAt.LookAtTargetTypes.YawPitchValue:
                    return (Yaw, Pitch);
                default:
