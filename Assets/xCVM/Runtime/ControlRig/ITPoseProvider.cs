...
namespace xCVM
{
    ...
    public static class ITPoseProviderExtensions
    {
        static HumanBodyBones GetParent(ITPoseProvider self, HumanBodyBones bone)
        {
            ...
            for (int i = 0; i < 100; ++i)
            {
                var vrmBone = xCVMHumanoidBoneSpecification.ConvertFromUnityBone(current);
                var def = xCVMHumanoidBoneSpecification.GetDefine(vrmBone);
                var parentBone = xCVMHumanoidBoneSpecification.ConvertToUnityBone(def.ParentBone.Value);
