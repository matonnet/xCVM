...
using UnityEngine;
using VRMShaders;
using VRMShaders.xCVM.MToon10.Runtime;

namespace xCVM
{
    public static class xCVMMToonTextureImporter
    {
        ...
        {
            return TryGetSRGBTexture(data, new xCVMTextureInfo(mToon.ShadeMultiplyTexture), out key, out desc);
        }
        ...
        {
            return TryGetLinearTexture(data, new xCVMTextureInfo(mToon.ShadingShiftTexture), out key, out desc);
        }
        ...
        {
            return TryGetSRGBTexture(data, new xCVMTextureInfo(mToon.MatcapTexture), out key, out desc);
        }
        ...
        {
            return TryGetSRGBTexture(data, new xCVMTextureInfo(mToon.RimMultiplyTexture), out key, out desc);
        }
        ...
        {
            return TryGetLinearTexture(data, new xCVMTextureInfo(mToon.OutlineWidthMultiplyTexture), out key, out desc);
        }
        ...
        {
            return TryGetLinearTexture(data, new xCVMTextureInfo(mToon.UvAnimationMaskTexture), out key, out desc);
        }

        private static bool TryGetSRGBTexture(GltfData data, xCVMTextureInfo info, out SubAssetKey key, out TextureDescriptor desc)
        {
            try
            {
        ...
        private static bool TryGetLinearTexture(GltfData data, xCVMTextureInfo info, out SubAssetKey key, out TextureDescriptor desc)
        {
            try
            {
                var (offset, scale) = GetTextureOffsetAndScale(info);
        ...
        private static (Vector2, Vector2) GetTextureOffsetAndScale(xCVMTextureInfo textureInfo)
        {
        ...
        /// </summary>
        private sealed class xCVMTextureInfo : glTFTextureInfo
        {
            public xCVMTextureInfo(TextureInfo info)
            {
                if (info == null) return;
            ...
            public xCVMTextureInfo(ShadingShiftTextureInfo info)
            {
                if (info == null) return;
