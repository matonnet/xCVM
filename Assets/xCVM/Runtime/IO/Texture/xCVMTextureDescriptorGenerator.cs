...
namespace xCVM
{
    public sealed class xCVMTextureDescriptorGenerator : ITextureDescriptorGenerator
    {
        public const string UniqueThumbnailName = "thumbnail__xCVM";

        private readonly GltfData m_data;
        private TextureDescriptorSet _textureDescriptorSet;

        public xCVMTextureDescriptorGenerator(GltfData data)
        {
            m_data = data;
        }
        ...
        private static IEnumerable<(SubAssetKey, TextureDescriptor)> EnumerateAllTextures(GltfData data)
        {
            ...
            for (var materialIdx = 0; materialIdx < data.GLTF.materials.Count; ++materialIdx)
            {
                ...
                {
                    foreach (var (_, tex) in xCVMMToonTextureImporter.EnumerateAllTextures(data, m, mToon))
                    {
                        yield return tex;
                    }
                }
