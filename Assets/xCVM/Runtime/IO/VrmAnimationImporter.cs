    ...
    public class VrmAnimationImporter : UniGLTF.ImporterContext
    {
        ...
        IEnumerable<ExpressionInfo> IterateExpressions()
        {
            if (m_vrma is UniGLTF.Extensions.VRMC_vrm_animation.VRMC_vrm_animation animation && animation.Expressions != null)
            {
                ...
                {
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.happy), nameof(xCVMAnimationInstance.preset_happy), preset.Happy) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.angry), nameof(xCVMAnimationInstance.preset_angry), preset.Angry) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.sad), nameof(xCVMAnimationInstance.preset_sad), preset.Sad) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.relaxed), nameof(xCVMAnimationInstance.preset_relaxed), preset.Relaxed) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.surprised), nameof(xCVMAnimationInstance.preset_surprised), preset.Surprised) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.aa), nameof(xCVMAnimationInstance.preset_aa), preset.Aa) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.ih), nameof(xCVMAnimationInstance.preset_ih), preset.Ih) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.ou), nameof(xCVMAnimationInstance.preset_ou), preset.Ou) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.ee), nameof(xCVMAnimationInstance.preset_ee), preset.Ee) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.oh), nameof(xCVMAnimationInstance.preset_oh), preset.Oh) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.blink), nameof(xCVMAnimationInstance.preset_blink), preset.Blink) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.blinkLeft), nameof(xCVMAnimationInstance.preset_blinkleft), preset.BlinkLeft) is ExpressionInfo info) yield return info; }
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.blinkRight), nameof(xCVMAnimationInstance.preset_blinkright), preset.BlinkRight) is ExpressionInfo info) yield return info; }
                    ...
                    { if (GetExpression(gltfAnimation, ExpressionKey.CreateFromPreset(ExpressionPreset.neutral), nameof(xCVMAnimationInstance.preset_neutral), preset.Neutral) is ExpressionInfo info) yield return info; }
                }
            ...
            if (expressions.Length > 0)
            {
                ...
                foreach (var expression in expressions)
                {
                    ...
                    var curve = CreateCurve(
                        input.ToArray(),
                        output.ToArray());
                    clip.SetCurve("", typeof(xCVMAnimationInstance), expression.PropertyName, curve);
                }
            }

            // VRMA-animation solver
            var animationInstance = instance.gameObject.AddComponent<xCVMAnimationInstance>();
