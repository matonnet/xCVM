...
namespace xCVM
{
    public class xCVMExporter : IDisposable
    {
        ...
        public xCVMExporter(ITextureSerializer textureSerializer, GltfExportSettings settings)
        {
            m_settings = settings;
        ...
        {
            var materialExporter = new BuiltInxCVMMaterialExporter();
        ...
        public void Export(GameObject root, Model model, ModelExporter converter, ExportArgs option, xCVMObjectMeta vrmMeta = null)
        {
        ...
        xCVMObjectMeta vrmMeta, List<glTFNode> nodes, ITextureExporter textureExporter)
        {
            var vrmController = root?.GetComponent<xCVMInstance>();
        ...
        static UniGLTF.Extensions.VRMC_springBone.ColliderShape ExportShape(xCVMSpringBoneCollider z)
        {
        ...
        static UniGLTF.Extensions.VRMC_springBone.SpringBoneJoint ExportJoint(xCVMSpringBoneJoint y, Func<Transform, int> getIndexFromTransform)
        {
        ...
        static UniGLTF.Extensions.VRMC_springBone.VRMC_springBone ExportSpringBone(xCVMInstance controller, Model model, ModelExporter converter)
        {
            var colliders = controller.GetComponentsInChildren<xCVMSpringBoneCollider>();
        ...
        static void ExportConstraints(xCVMInstance vrmController, Model model, ModelExporter converter, List<glTFNode> nodes)
        {
            var constraints = vrmController.GetComponentsInChildren<IxCVMConstraint>();
            foreach (var constraint in constraints)
            {
                ...
                switch (constraint)
                {
                    case xCVMAimConstraint aimConstraint:
                        vrmConstraint.Constraint.Aim = new UniGLTF.Extensions.VRMC_node_constraint.AimConstraint
                        {
                            ...
                            AimAxis = xCVMConstraintUtil.ReverseX(aimConstraint.AimAxis),
                        };
                        break;

                    case xCVMRollConstraint rollConstraint:
                    ...
                    case xCVMRotationConstraint rotationConstraint:
        ...
        void ExportFirstPerson(UniGLTF.Extensions.VRMC_vrm.VRMC_vrm vrm, xCVMInstance vrmController, Model model, ModelExporter converter)
        {
            if (!(vrmController?.Vrm?.FirstPerson is xCVMObjectFirstPerson firstPerson))
            {
                return;
            }
        ...
        static void ExportLookAt(UniGLTF.Extensions.VRMC_vrm.VRMC_vrm vrm, xCVMInstance vrmController)
        {
            if (!(vrmController?.Vrm?.LookAt is xCVMObjectLookAt lookAt))
            {
                return;
            }
        ...
        static UniGLTF.Extensions.VRMC_vrm.Expression ExportExpression(xCVMExpression e, xCVMInstance vrmController, Model model, ModelExporter converter)
        {
            if (e == null)
            {
                return null;
        ...
        static void ExportExpression(UniGLTF.Extensions.VRMC_vrm.VRMC_vrm vrm, xCVMInstance vrmController, Model model, ModelExporter converter)
        {
        ...
        public static int? ExportMeta(UniGLTF.Extensions.VRMC_vrm.VRMC_vrm vrm, xCVMObjectMeta meta, ITextureExporter textureExporter)
        {
        ...
        /// <returns></returns>
        public static byte[] Export(GameObject go, ITextureSerializer textureSerializer = null, xCVMObjectMeta vrmMeta = null)
        {
            using (var arrayManager = new NativeArrayManager())
            {
                // ヒエラルキーからジオメトリーを収集
                var converter = new xCVM.ModelExporter();
                ...
                var exporter10 = new xCVMExporter(textureSerializer ?? new RuntimeTextureSerializer(), new GltfExportSettings());
