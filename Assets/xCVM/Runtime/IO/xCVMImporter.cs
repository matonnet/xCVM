...
namespace xCVM
{
    ...
    /// </summary>
    public class xCVMImporter : UniGLTF.ImporterContext
    {
        private readonly xCVMData m_vrm;
        ...
        private xCVMObject m_vrmObject;
        private List<(ExpressionPreset Preset, xCVMExpression Clip)> m_expressions = new List<(ExpressionPreset, xCVMExpression)>();
            ...
            : base(vrm.Data, externalObjectMap, textureDeserializer)
        {
            ...
            TextureDescriptorGenerator = new xCVMTextureDescriptorGenerator(Data);
            MaterialDescriptorGenerator = materialGenerator ?? xCVMRenderPipelineMaterialDescriptorGeneratorDescriptorUtility.GetValidxCVMMaterialDescriptorGenerator();
        ...
        protected override async Task OnLoadHierarchy(IAwaitCaller awaitCaller, Func<string, IDisposable> MeasureTime)
        {
            ...
            // VrmController
            var controller = Root.AddComponent<xCVMInstance>();
        ...
        xCVMExpression GetOrLoadExpression(in SubAssetKey key, ExpressionPreset preset, UniGLTF.Extensions.VRMC_vrm.Expression expression)
        {
            xCVMExpression clip = default;
            if (m_externalMap.TryGetValue(key, out UnityEngine.Object expressionObj))
            {
                clip = expressionObj as xCVMExpression;
            }
            else
            {
                if (expression == null)
                ...
                clip = ScriptableObject.CreateInstance<xCVM.xCVMExpression>();
                clip.name = key.Name;
        ...
        public async Task<Texture2D> LoadVrmThumbnailAsync(IAwaitCaller awaitCaller = null)
        {
            ...
            if (xCVMTextureDescriptorGenerator.TryGetMetaThumbnailTextureImportParam(Data, m_vrm.VrmExtension, out (SubAssetKey, VRMShaders.TextureDescriptor Param) kv))
            {
        ...
        async Task<xCVMObject> LoadVrmAsync(IAwaitCaller awaitCaller, UniGLTF.Extensions.VRMC_vrm.VRMC_vrm vrmExtension)
        {
            if (m_externalMap.TryGetValue(xCVMObject.SubAssetKey, out UnityEngine.Object obj) && obj is xCVMObject vrm)
            {
            ...
            // create new object
            m_vrmObject = vrm = ScriptableObject.CreateInstance<xCVMObject>();
            vrm.name = xCVMObject.SubAssetKey.Name;

            // meta
            if (vrmExtension.Meta != null)
            {
                var src = vrmExtension.Meta;
                var meta = new xCVMObjectMeta();
                vrm.Meta = meta;
                meta.Name = src.Name;
        ...
        async Task LoadSpringBoneAsync(IAwaitCaller awaitCaller, xCVMInstance controller, UniGLTF.Extensions.VRMC_springBone.VRMC_springBone gltfVrmSpringBone)
        {
            await awaitCaller.NextFrame();

            // colliders
            var colliders = new List<xCVMSpringBoneCollider>();
            if (gltfVrmSpringBone.Colliders != null)
            {
                foreach (var c in gltfVrmSpringBone.Colliders)
                {
                    var collider = Nodes[c.Node.Value].gameObject.AddComponent<xCVMSpringBoneCollider>();
                    colliders.Add(collider);
                    if (c.Shape.Capsule is UniGLTF.Extensions.VRMC_springBone.ColliderShapeCapsule capsule)
                    {
                        collider.ColliderType = xCVMSpringBoneColliderTypes.Capsule;
                    ...
                    {
                        collider.ColliderType = xCVMSpringBoneColliderTypes.Sphere;
                        collider.Offset = Vector3InvertX(sphere.Offset);
                        collider.Radius = sphere.Radius.Value;
                    }
                    else
                    {
                        throw new xCVMException("unknown shape");
                    }
                }
                ...
                if (gltfVrmSpringBone.ColliderGroups != null)
                {
                    ...
                    {
                        var colliderGroup = secondary.gameObject.AddComponent<xCVMSpringBoneColliderGroup>();
            ...
            if (gltfVrmSpringBone.Springs != null)
            {
                ...
                foreach (var gltfSpring in gltfVrmSpringBone.Springs)
                {
                    ...
                    var spring = new xCVMInstanceSpringBone.Spring(gltfSpring.Name);
                    ...
                    foreach (var gltfJoint in gltfSpring.Joints)
                    {
                        if (gltfJoint.Node.HasValue)
                        {
                            ...
                            var joint = go.GetComponent<xCVMSpringBoneJoint>();
                            if (joint != null)
                            {
                            ...
                            joint = go.AddComponent<xCVMSpringBoneJoint>();
        ...
        /// <returns></returns>
        async Task LoadConstraintAsync(IAwaitCaller awaitCaller, xCVMInstance controller)
        {
            for (int i = 0; i < Data.GLTF.nodes.Count; ++i)
            {
                ...
                if (UniGLTF.Extensions.VRMC_node_constraint.GltfDeserializer.TryGet(gltfNode.extensions, out UniGLTF.Extensions.VRMC_node_constraint.VRMC_node_constraint ext))
                {
                    ...
                    if (constraint.Roll != null)
                    {
                        ...
                        var component = node.gameObject.AddComponent<xCVMRollConstraint>();
                    ...
                    else if (constraint.Aim != null)
                    {
                        ...
                        var component = node.gameObject.AddComponent<xCVMAimConstraint>();
                        component.Source = Nodes[aim.Source.Value]; // required
                        component.Weight = aim.Weight.GetValueOrDefault(1.0f);
                        component.AimAxis = xCVMConstraintUtil.ReverseX(aim.AimAxis); // required
                    }
                    else if (constraint.Rotation != null)
                    {
                        var rotation = constraint.Rotation;
                        var component = node.gameObject.AddComponent<xCVMRotationConstraint>();
        ...
        public override void TransferOwnership(TakeResponsibilityForDestroyObjectFunc take)
        {
            ...
            if (m_vrmObject != null)
            {
                take(xCVMObject.SubAssetKey, m_vrmObject);
                m_vrmObject = null;
            }
