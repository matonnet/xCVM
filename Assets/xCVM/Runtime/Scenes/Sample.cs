...
namespace xCVM.Sample
{
    ...
    public class Sample : MonoBehaviour
    {
        ...
        async void Run()
        {
            ...
            var instance = await xCVM.LoadPathAsync(m_vrmPath, true);

            var exportedBytes = xCVMExporter.Export(instance.gameObject);

            // Import 1.0
            var xCVM = await xCVM.LoadBytesAsync(exportedBytes, false);
            var pos = xCVM.transform.position;
            pos.x += 1.5f;
            xCVM.transform.position = pos;
            xCVM.name = xCVM.name + "_Imported_v1_0";

            // write
            var path = Path.GetFullPath("xCVM.vrm");
