...
{
    public class xCVMRuntimeLoader : MonoBehaviour
    {
        [SerializeField, Header("GUI")]
        xCVMCanvasManager m_canvas = default;

        [SerializeField]
        xCVMLookTarget m_faceCamera = default;
        ...
        async Task<xCVMInstance> LoadAsync(string path, VRMShaders.IAwaitCaller awaitCaller)
        {
            var instance = await xCVM.LoadPathAsync(path, awaitCaller: awaitCaller);
        ...
        {
#if UNITY_STANDALONE_WIN
            var path = xCVMFileDialogForWindows.FileDialog("open BVH", ".bvh");
            if (!string.IsNullOrEmpty(path))
            {
                LoadBvh(path);
            }
