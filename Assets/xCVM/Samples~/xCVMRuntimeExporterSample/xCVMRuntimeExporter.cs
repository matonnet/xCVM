...
namespace xCVM.RuntimeExporterSample
{
    public class xCVMRuntimeExporter : MonoBehaviour
    {
        [SerializeField]
        xCVMObjectMeta m_meta = new xCVMObjectMeta();
        ...
        async void Load()
        {
            ...
            var xCVM = await xCVM.LoadPathAsync(path);
            var loaded = xCVM.GetComponent<UniGLTF.RuntimeGltfInstance>();
            loaded.ShowMeshes();
        ...
        static byte[] ExportSimple(GameObject root, xCVMObjectMeta meta)
        {
            using (var arrayManager = new NativeArrayManager())
            {
                ...
                // export vrm-1.0
                var exporter = new xCVM.xCVMExporter(new RuntimeTextureSerializer(), new GltfExportSettings
                {
